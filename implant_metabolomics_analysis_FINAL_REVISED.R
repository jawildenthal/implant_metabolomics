...
hexsphDot <- monochromeDotplotYN("pos_461.33469_13.882", myPval = plotsignifstars(aucdf$fdrMW[aucdf$cid=="pos_461.33469_13.882"]), mytitle = "Glucosyl-sphingosine")
...
ggdraw() +
  ...
  draw_plot(makeLongitudinalDotplot("pos_461.33469_13.882",mytitle="Glucosyl-sphingosine", fdrvec=getfdrsfromchart("pos_461.33469_13.882")), x = 0.5, y = 1/2, width = 0.5, height = 1/4) +
...
instudyPseud["pseudoStatusV2"] <- sapply(instudyPseud$xbrID, function(i){paste0(xbrkey$sampletype[xbrkey$runID==i],"_",ifelse(!is.na(xbrkey$bacteriaIsolate_ALL[xbrkey$runID==i]),"C+","C-"),"_",ifelse(xbrkey$`Pseudomonas aeruginosa`[xbrkey$runID==i],"PA+","PA-"))})
instudyPseud$pseudoStatusV2 <- factor(instudyPseud$pseudoStatusV2, levels = c("drain_C-_PA-","drain_C+_PA-","drain_C+_PA+","seroma_C-_PA-","seroma_C+_PA-","seroma_C+_PA+"))


makepseudoplotV2 <- function(CID,myblanks=posblanks, pseudname=CID, mydf=instudyPseud){
  mydf <- instudyPseud
  ...
  p2<-ggplot(mydf, aes(x=pseudoStatusV2, y=.data[[CID]])) + 
    mytheme+
    geom_boxplot(fill=NA, color=c("grey40","grey40","lightseagreen","grey40","grey40","lightseagreen"), outliers=FALSE, linewidth=0.7)+
    ...
    scale_y_continuous(limits = c(minval-0.2*rangeCID, maxval+0.6*rangeCID), breaks = seq(floor(min(mydf[,CID])-0.5),ceiling(max(mydf[,CID])+0.5), by = 1))+
    scale_x_discrete(labels = c("drain_C-_PA-" = "D\nC-\nPA-","drain_C+_PA-"= "D\nC+\nPA-","drain_C+_PA+"= "D\nC+\nPA+","seroma_C-_PA-"= "S\nC-\nPA-","seroma_C+_PA-"="S\nC+\nPA-","seroma_C+_PA+"="S\nC+\nPA+"))+
    #theme(aspect.ratio = 2/1, plot.margin = margin(t = 0.1, r = 0.1, b = 0.1, l = 0.1, unit = "cm"))+
    geom_hline(yintercept=mean(myblanks[,CID]), linetype='dotted', col = "black")+
    geom_signif(
      comparisons = list(c("drain_C-_PA-", "drain_C+_PA+"),c("drain_C+_PA-", "drain_C+_PA+"),c("seroma_C-_PA-", "seroma_C+_PA+"),c("seroma_C+_PA-", "seroma_C+_PA+")),#,c("drain_PA+","seroma_PA+")),
      map_signif_level = TRUE,
      textsize=2.8224,
      family="Helvetica",
      y_position = c(max(mydf[,CID])+0.02*rangeCID, 
                     max(mydf[,CID])+0.3*rangeCID,
                     max(mydf[,CID])+0.02*rangeCID, 
                     max(mydf[,CID])+0.3*rangeCID),
      #max(mydf[,CID])+0.2*rangeCID),
      color="black",
      annotation=c(
        plotsignifstars(wilcox.test(mydf[mydf$pseudoStatusV2=="drain_C-_PA-",CID],mydf[mydf$pseudoStatusV2=="drain_C+_PA+",CID])$p.value),
        plotsignifstars(wilcox.test(mydf[mydf$pseudoStatusV2=="drain_C+_PA-",CID],mydf[mydf$pseudoStatusV2=="drain_C+_PA+",CID])$p.value),
        plotsignifstars(wilcox.test(mydf[mydf$pseudoStatusV2=="seroma_C-_PA-",CID],mydf[mydf$pseudoStatusV2=="seroma_C+_PA+",CID])$p.value),
        plotsignifstars(wilcox.test(mydf[mydf$pseudoStatusV2=="seroma_C+_PA-",CID],mydf[mydf$pseudoStatusV2=="seroma_C+_PA+",CID])$p.value)
...
finalpseudomonalCIDs <- c("pos_650.38711_16.274","pos_504.32958_16.273","pos_324.06017_13.565","pos_210.07921_6.48")
finalpseudnames <- c("Di-rhamnolipid","Mono-rhamnolipid","Pyochelin","Pyocyanin")

pseudarray <- lapply(1:4, function(i){makepseudoplotV2(finalpseudomonalCIDs[i], pseudname = finalpseudnames[i])})


tiff("figures_and_tables/figure_7_FINAL.tiff", width=3.4,height=3.4, units="in",res=600)
ggdraw() +
  draw_plot(pseudarray[[1]], x = 0, y = 0.5, width = 0.5, height = 0.5) +
  draw_plot(pseudarray[[2]], x = 0.5, y = 0.5, width = 0.5, height = 0.5) +
  draw_plot(pseudarray[[3]], x = 0, y = 0, width = 0.5, height = 0.5) +
  draw_plot(pseudarray[[4]], x = 0.5, y = 0, width = 0.5, height = 0.5) +
  draw_plot_label(label = c("A", "B", "C","D"), size = 15,
                  x = c(0, 0.5, 0,0.5), y = c(1, 1, 0.5,0.5))
dev.off()

sdv_7a <- instudyPseud[,c("pseudoStatusV2","pos_650.38711_16.274")]
sdv_7b <- instudyPseud[,c("pseudoStatusV2","pos_504.32958_16.273")]
sdv_7c <- instudyPseud[,c("pseudoStatusV2","pos_324.06017_13.565")]
sdv_7d <- instudyPseud[,c("pseudoStatusV2","pos_210.07921_6.48")]
...
write.csv(sdv_7d, "supporting_data_values/sdv_7d.csv")
###################################################################################################################################
#FIGURE S5,6,8
#Cefazolin
